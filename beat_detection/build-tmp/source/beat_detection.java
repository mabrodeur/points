...
int maxTime = 200;
float predictiveInfluenceConstant = .1f;
...
public void draw() {
  ...
  int w = PApplet.parseInt(width/fft.avgSize());                             //Scales the FFT
  ...
  for (int i = 0; i < beatBands; i += 1) {
    totalLong[i] = totalLong[i]/(PApplet.parseFloat(longTermAverageSamples)/PApplet.parseFloat(shortTermAverageSamples));
    ...
    c[i] = 1.3f + constrain(map(delta[i], 0, 3000, 0, .4f), 0, .4f) + //delta is usually bellow 2000
    map(constrain(pow(totalLong[i], .5f), 0, 6), 0, 20, .3f, 0) +    //possibly comment this out, adds weight to the lower end
    map(constrain(count[i], 0, 15), 0, 15, 1, 0) - 
    map(constrain(count[i], 30, 200), 30, 200, 0, .75f);
    
 
    if (cyclePerBeatIntensity/standardDeviation > 3.5f){
      predictiveInfluence = predictiveInfluenceConstant * (1 - cos((PApplet.parseFloat(beatCounter)*TWO_PI)/PApplet.parseFloat(cyclesPerBeat)));
  ...
  rect(width - 100, height, width, height - beat/(PApplet.parseFloat(beatBands)/100));   //beat monitor
...
public void backgroundChange(int a) {              //Randomly changes background color
  redBackground = PApplet.parseInt(random(a));
  greenBackground = PApplet.parseInt(random(a));
  blueBackground = PApplet.parseInt(random(a));
}
...
public void stop()                                //Closes everything on stop
{
...
class Points {
  ...
  float vitesse;
  boolean actif=false;
  Points (float X, float Y,float L,float H,int C, boolean b ){ 
    vitesse=random(0.1f,1);
